...
while nsteps<max_steps:
    ...
    for x,y in steps:
        #rechts 
        if grid[y%SIZE,(x+1)%SIZE]==0 and str(x+1)+','+str(y) not in visited[(nsteps)%2]:
            visited[(nsteps)%2].update({str(x+1)+','+str(y):nsteps})    
            new_steps.append([x+1,y])
            #links
        ...
        if grid[(y+1)%SIZE,x%SIZE]==0 and str(x)+','+str(y+1) not in visited[(nsteps)%2]:
            visited[(nsteps)%2].update({str(x)+','+str(y+1):nsteps})      
            new_steps.append([x,y+1])       
            #boven
        if grid[(y-1)%SIZE,x%SIZE]==0 and str(x)+','+str(y-1) not in visited[(nsteps)%2]:
            visited[(nsteps)%2].update({str(x)+','+str(y-1):nsteps})       
    ...
    if nsteps==142:
        for key,value in visited[0].items():
            x,y=key.split(',')
            x=int(x)
            y=int(y)
            if x>=0 and x<SIZE and y>=0 and y<SIZE:
                even_vlak+=1
    if nsteps==143:
        for key,value in visited[1].items():
            x,y=key.split(',')
            x=int(x)
            y=int(y)
            if x>=0 and x<SIZE and y>=0 and y<SIZE:
